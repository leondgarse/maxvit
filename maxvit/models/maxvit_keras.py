...
class TrailDense(tf.keras.layers.Layer):
  ...
  def __init__(self,
               ...
               name='dense'):
    super(TrailDense, self).__init__()
  ...
  def get_config(self):
    ...
    config.update({"output_trailing_dims": self._output_trailing_dims, "begin_axis": self.begin_axis, "use_bias": self.use_bias})
    return config


class RelativeBias(tf.keras.layers.Layer):
  def __init__(self, rel_bias_shape, height, width, h_axis):
    super().__init__()
    self.rel_bias_shape, self.height, self.width, self.h_axis = rel_bias_shape, height, width, h_axis

  def build(self, input_shape):
    height, width = self.height, self.width
    self.relative_bias = self.add_weight('relative_bias', self.rel_bias_shape, trainable=True)
    self.__reindexed_bias__ = attn_utils.reindex_2d_einsum_lookup(self.relative_bias, height, width, height - 1, width - 1, h_axis=self.h_axis)

  def call(self, inputs, **kwargs):
    return inputs + self.__reindexed_bias__

  def get_config(self):
    config = super().get_config()
    config.update({
      "rel_bias_shape": self.rel_bias_shape,
      "height": self.height,
      "width": self.width,
      "h_axis": self.h_axis,
    })
...
class Attention(tf.keras.layers.Layer):
  ...
  def build(self, query_shape):
    ...
    self._q_proj = TrailDense(output_trailing_dims=[self.num_heads, self.head_size])
    self._k_proj = TrailDense(output_trailing_dims=[self.num_heads, self.head_size])
    self._v_proj = TrailDense(output_trailing_dims=[self.num_heads, self.head_size])
    self._o_proj = TrailDense(output_trailing_dims=self.hidden_size, begin_axis=-2)
    ...
    if self.rel_attn_type in ['2d_multi_head', '2d_single_head']:
      ...
      if self.rel_attn_type == '2d_multi_head':
        ...
        raise NotImplementedError('rel_attn_type %s not implemented yet.' %
                                  self.rel_attn_type)
      self.reindexed_bias = RelativeBias(rel_bias_shape, height, width, h_axis)
    else:
      self.reindexed_bias = None

  def __call__(self, query, training, context=None, attn_mask=None):
    if not self.built:
    ...
    q_heads = tf.transpose(q_heads, [0, 2, 1, 3])
    k_heads = tf.transpose(k_heads, [0, 2, 3, 1])
    v_heads = tf.transpose(v_heads, [0, 2, 1, 3])

    # attention
    # attn_logits = tf.einsum(f'{self.q_expr},{self.k_expr}->{self.a_expr}', q_heads, k_heads)
    q_heads *= self.q_scale
    attn_logits = q_heads @ k_heads
    # print(f">>>> {q_heads.shape = }, {k_heads.shape = }, {attn_logits.shape = }")

    if self.reindexed_bias is not None:
      attn_logits = self.reindexed_bias(attn_logits)

    if attn_mask is not None:
      attn_logits += (1.0 - attn_mask) * attn_logits.dtype.min

    # attn_probs = float32_softmax(attn_logits, axis=-1)
    attn_probs = tf.nn.softmax(attn_logits, axis=-1)
    if self.dropatt:
      attn_probs = tf.keras.layers.Dropout(self.dropatt, 'attn_prob_drop')(attn_probs, training=training)

    # attn_out = tf.einsum(f'{self.a_expr},{self.v_expr}->{self.q_expr}', attn_probs, v_heads)
    attn_out = attn_probs @ v_heads
    attn_out = tf.transpose(attn_out, [0, 2, 1, 3])
    # print(f">>>> {attn_probs.shape = }, {v_heads.shape = }, {attn_out.shape = }")
    output = self._o_proj(attn_out)
    # print(f">>>> {self._o_proj.einsum_expr = }, {attn_out.shape = }, {self._o_proj.weight.shape = }")
...
class FFN(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self._shrink_dense = TrailDense(
        ...
        bias_initializer=bias_initializer,
        name='shrink_dense')
    self._activation_fn = tf.keras.layers.Activation(self.activation)
  ...
  def call(self, inputs, training):
    ...
    if self.dropout:
      output = tf.keras.layers.Dropout(self.dropout)(
...
class TransformerBlock(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training, attn_mask=None):
    ...
    if config.dropout:
      output = tf.keras.layers.Dropout(config.dropout)(
          output, training=training)
...
class MBConvBlock(tf.keras.layers.Layer):
  ...
  def __init__(self, config):
    ...
    self._activation_fn = tf.nn.gelu
  ...
  def build(self, input_shape):
    ...
    self._depthwise_conv = tf.keras.layers.DepthwiseConv2D(
        ...
        data_format=config.data_format,
        use_bias=False)
    self._depthwise_norm = norm_class()
  ...
  def call(self, inputs, training, survival_prob=None):
    ...
    output = tf.keras.layers.Add()([residual, shortcut])
...
class MaxViTBlock(tf.keras.layers.Layer):
  ...
  def block_ffn_branch(self, inputs, training):
    ...
    if not self._block_ffn.built:
      self._block_ffn.build(output.shape)
      self._block_ffn.built = True
    output = self._block_ffn.call(output, training)
    return output

  def grid_ffn_branch(self, inputs, training):
    output = self._grid_ffn_layer_norm(inputs)
    if not self._grid_ffn.built:
      self._grid_ffn.build(output.shape)
      self._grid_ffn.built = True
    output = self._grid_ffn.call(output, training)
    return output

  def mbconv_branch(self, inputs, training):
    if not self._mbconv.built:
      self._mbconv.build(inputs.shape)
      self._mbconv.built = True
    output = self._mbconv.call(inputs, training=training)
    return output
...
class MaxViT(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training):
    ...
    if not self.built:
      self.build(inputs.shape)
      self.built = True
    output = self._stem.call(inputs, training=training)
    ...
    for idx, stage_blocks in enumerate(self._blocks):
      ...
      for block in stage_blocks:
        if not block.built:
          block.build(output.shape)
          block.built = True
        output = block.call(output, training=training)
    ...
    # classification head
    output = self._cls_head.call(output, training=training)
